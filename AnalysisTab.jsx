...
// ── Lab-wide Top Applications ────────────────────────────────────────────────────
const LabTopApplications = () => {
  ...
  const maxBytes = applications[0]?.bytes || 1;
  const totalBytes = applications.reduce((s, d) => s + d.bytes, 0);
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4">
      {loading ? (
        ...
        <p className="text-center text-gray-400 text-sm py-4">No application data yet</p>
      ) : (
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-[10px] text-gray-400 uppercase font-bold pb-1 border-b border-gray-100">
            ...
            <span className="w-44">Application</span>
            <span className="flex-1">Traffic Share</span>
            <span className="w-20 text-right">Total</span>
            <span className="w-12 text-right">%</span>
          </div>
          {applications.map(({ name, bytes }, i) => {
            ...
            return (
              <div key={name} className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-700 font-mono w-44 truncate shrink-0">{name}</span>
...
const AnalysisTab = ({ selectedLab, filteredPCs }) => {
  ...
  return (
    <div className="w-full pb-8">
      ...
      {/* Top Visited Applications - combined across all PCs */}
      <h2 className="text-base font-black text-gray-700 uppercase tracking-wider mb-3 mt-6">Top Visited Applications</h2>
      <LabTopApplications />
