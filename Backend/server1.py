    ...
    if packet.haslayer(Ether):
        ...
        for mac in [src_mac, dst_mac]:
            if mac in mac_addresses:
                ...
                domain = domain_ips.get(target_ip)
...
def start_sniffing():
    ...
    while capturing_flag:
        try:
            sniff(prn=process_packet, iface="Wi-Fi", timeout=1, store=False)
...
def get_formatted_data():
    result = {}
    for mac, dirs in mac_traffic.items():
        result[mac] = {"upload": {}, "download": {}}
        for direction in ["upload", "download"]:
            for domain, info in dirs[direction].items():
                result[mac][direction][domain] = {
                    "ips": info["ips"],
...
def get_mac_data(mac_address):
    ...
    if mac_address in data_usage:
        ...
        return jsonify({
            ...
            "device_ip": mac_to_ip.get(mac_address, "Unknown")
        })
    else:
